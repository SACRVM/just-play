...
public static class ChannelExtractor
{
    ...
    /// </summary>
    /// <param name="source">Interleaved source PCM (channels × frames).</param>
    ...
    /// <returns>The number of valid floats written to <paramref name="dest"/> (frames × 2).</returns>
    public static int ToStereoPair(float[] source, int validFloats, int channels, int masterOffset, ref float[] dest)
    {
        ...
        // Keep the pair inside the buffer: offset ∈ [0, channels-2], even (pairs are L/R aligned).
