...
/// L/R meters, and the errors-only log. Layout it drives: just-stream-blueprint.md Sec.3a / Sec.7.3.
///
...
public sealed partial class StreamViewModel : ObservableObject, IDisposable
{
    ...
    /// <summary>
    /// The transport mark on the quiet-glass CTA (Button.cta in App.axaml): play = go live, stop =
    /// come off air; nothing at all while it is connecting, because the word carries that state.
    ///
    /// <para>It used to be baked into <see cref="ConnectLabel"/> as ">" / "stop". Those are characters,
    /// and a character is drawn by the font the OS picks - on macOS both have emoji presentation by
    /// default, so the mark would have come out coloured there, on a pill whose whole point is that
    /// only OUR colour changes. Shipped vectors instead (CLAUDE.md rule 5).</para>
    /// </summary>
    public IconKind ConnectIcon => State switch
    {
        BroadcastState.Connected => IconKind.Stop,
        BroadcastState.Connecting or BroadcastState.Reconnecting => IconKind.None,
        _ => IconKind.Play,
    };
    ...
    // -- Live readouts (direct fields, never the log - Sec.3a) ----------------
    ...
    /// <summary>
    /// Limiter/maximizer drive mapping (just-stream-blueprint.md Sec.4): Off = bypass / Soft = 0 dB
    ...
    // -- Log (errors/warnings only - Sec.3a) - feeds the shared EventLog (JustPlay.UI) --------
