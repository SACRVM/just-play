...
/// <list type="bullet">
///   <item><b>Fast follower</b> (tau = 2 ms): catches the leading edge of a transient within a
///     few milliseconds and drops back to zero within ~20 ms.</item>
///   <item><b>Slow follower</b> (tau = 50 ms): lags behind on attack onsets; represents the
...
/// <code>gain = 1 + Punch x attackAmount x K</code>
...
/// (equal attack and release tau), so each acts as a linear first-order IIR low-pass on the
/// detector signal.  At any frequency well above 1/tau, a symmetric IIR peak follower converges
/// to the time-average of |signal| = 2A/pi regardless of its time constant.  After a warmup
/// period (~ 5 x tau_slow ~ 250 ms), both fastEnv and slowEnv converge to the same DC level
/// (2A/pi).  The only residual difference is the small ripple the fast follower retains (larger
/// tau -> less filtering at 2f), which is typically &lt; 1 % of the signal amplitude at frequencies
...
public sealed class TransientDesigner
{
    ...
    // Both followers are symmetric (tau_attack = tau_release = tau), making them linear IIR low-pass
    // filters: H(z) = (1-alpha)/(1-alphaz^-^1).  DC gain is exactly 1 regardless of tau, so both converge
    // to the time-average of |signal| = 2A/pi for any sustained periodic signal.  After warmup
    // (~ 5 x tau_slow = 250 ms), fastEnv ~ slowEnv in steady state -> diff -> 0 -> gain -> 1.0.
    ...
    private const double SlowMs = 50.0;   // 50 ms: background-level tracker; 5 tau = 250 ms to full decay

    private readonly double _fastCoeff;   // exp(-1 / (FastAttackMs x fs / 1000))
    private readonly double _slowCoeff;   // exp(-1 / (SlowAttackMs x fs / 1000))
    ...
    public void ProcessInterleavedStereo(Span<float> buf)
    {
        ...
        // equals the input with strict bit-for-bit transparency (no rounding from x1.0f).
        if (Punch == 0.0) return;
