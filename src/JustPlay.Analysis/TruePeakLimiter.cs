...
///
/// <para><b>Design reference:</b> streaming-broadcast.md Sec.R2, sections 5.1-5.3 and 7.2.</para>
///
...
///   <item>Upsamples each frame by 4x using a minimal polyphase linear-interpolation
///         kernel (per ITU-R BS.1770-4 Sec. 4.1 / streaming-broadcast.md Sec.5.2) to detect
...
///         This is the critical anti-pumping constraint from streaming-broadcast.md Sec.7.2.</item>
/// </list>
/// </para>
///
...
public sealed class TruePeakLimiter
{
    ...
    /// <summary>True-peak ceiling in dBTP. Default -1 dBTP per streaming-broadcast.md Sec.4.1.</summary>
    public double CeilingDbTp { get; }

    /// <summary>Lookahead delay in milliseconds. streaming-broadcast.md Sec.5.3 recommends 1-5 ms.</summary>
    ...
    /// Computed as exp(-1 / (attackMs x sampleRate / 1000)).
    /// streaming-broadcast.md Sec.5.3: "exponential decay on attack (fast)."
    /// </summary>
    private readonly double _attackCoeff;

    /// <summary>
    /// Release coefficient for the single-pole IIR gain envelope.
    /// Computed as exp(-1 / (releaseMs x sampleRate / 1000)).
    /// streaming-broadcast.md Sec.5.3: "slower release (~50-200 ms)."
    /// </summary>
    ...
    /// Initialized to 1.0 (no reduction). Per streaming-broadcast.md Sec.7.2 it never rises
    /// above 1.0 - gain ONLY reduces, never inflates.
    /// </summary>
    private double _gainEnvelope;

    // -------------------------------------------------------------------------
    // 4x oversampling - polyphase linear interpolation kernel
    // -------------------------------------------------------------------------
    ...
    // ITU-R BS.1770-4 reference uses a 48-tap FIR (12 taps x 4 phases).
    ...
    // [streaming-broadcast.md Sec.5.2 - "4x oversampled measurement"]
    //
    ...
    ///   True-peak ceiling in dBTP. Default -1.0 (EBU R128 / streaming-broadcast.md Sec.4.1).
    ///   Use -0.1 for a live-stream final stage with no downstream re-encode.
    /// </param>
    /// <param name="lookaheadMs">
    ///   Lookahead delay in ms. streaming-broadcast.md Sec.5.3 recommends 1-5 ms.
    ///   Default 2 ms - enough to prevent any transient overshoot on music material.
    /// </param>
    /// <param name="attackMs">
    ///   Gain-reduction attack time constant in ms. streaming-broadcast.md Sec.5.3:
    ///   "exponential decay on attack (fast, ~0.1-1 ms)." Default 0.5 ms.
    /// </param>
    /// <param name="releaseMs">
    ///   Gain-reduction release time constant in ms. streaming-broadcast.md Sec.5.3:
        ...
        double releaseMs    = 100.0)
    {
        ...
        // Lookahead: samples = ms x (sampleRate / 1000), minimum 1 sample.
        ...
        // [streaming-broadcast.md Sec.5.3 - exponential attack/release envelope]
    ...
    /// (streaming-broadcast.md Sec.7.2 - gain ONLY reduces, never inflates).</para>
    /// </summary>
    ...
    public void ProcessInPlace(Span<float> samples)
    {
        for (var i = 0; i < samples.Length; i++)
        {
            ...
            // -----------------------------------------------------------------
            // 1. True-peak detection: 4x oversample the local neighbourhood.
            ...
            //    [streaming-broadcast.md Sec.5.3 step 2c]
            // -----------------------------------------------------------------
            ...
            //    [streaming-broadcast.md Sec.5.3 step 3 - exponential envelope]
            //    [streaming-broadcast.md Sec.7.2 - monotone downward only]
            // -----------------------------------------------------------------
            ...
            //    [streaming-broadcast.md Sec.5.3 step 2d - apply to delayed signal]
            // -----------------------------------------------------------------
        ...
        float[] samples)
    {
        ...
        // Output true-peak (4x oversampled)
    ...
    /// <summary>
    /// Measures the 4x oversampled true-peak of <paramref name="samples"/> WITHOUT applying limiting.
    ...
    /// <summary>
    /// Upsamples the interval [<paramref name="s0"/>, <paramref name="s1"/>] by 4x
    /// using linear interpolation and returns the peak absolute magnitude over all
    /// 4 sub-samples (phases 0/4, 1/4, 2/4, 3/4).
    ///
    /// <para>Linear interpolation formula: s(t) = s0 + t-(s1 - s0), t  in  [0, 1).</para>
    ///
    ...
    /// not dangerous. [streaming-broadcast.md Sec.5.2 error budget note]</para>
    /// </summary>
