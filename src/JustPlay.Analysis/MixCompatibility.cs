...
// ============================================================================
// TUNE ON CHLOE'S EAR (Bradley-Terry, see harmonic-mixing.md §Personalisation)
//
...
// See harmonic-mixing.md §"Bradley-Terry = logistic regression on feature
...
/// </summary>
/// <param name="Combined">
/// Weighted-average compatibility ∈ [0, 1] (1 = perfectly compatible).
/// Computed from whichever sub-scores had data; unavailable axes are excluded
/// and surviving weights renormalised.
/// </param>
/// <param name="Beat">
/// Beat/groove similarity ∈ [0, 1], or null if no fingerprint available.
...
///   // r.Combined ∈ [0,1]; 1 = best possible mix.
///   // r.Beat / r.Tempo / r.Harmonic / r.Energy are the four sub-scores.
/// </code>
///
/// <para><b>Sub-score formulae (all ∈ [0,1]):</b></para>
/// <list type="bullet">
///   <item>
...
/// (see harmonic-mixing.md §"Personalisation").
///
/// <para>
...
public static class MixCompatibility
{
    ...
    /// <list type="number">
    ///   <item>
    ///     For each candidate ratio r ∈ {1, 2, 0.5, 1.5, 0.75}, compute
    ///     relDist = |1 − bpmA / (r × bpmB)|.  This measures how far bpmA
    ...
    /// harmonic-mixing.md §"Why the Camelot wheel works").
    /// </para>
    ///
    /// <para><b>Tier table (Camelot distance → score):</b></para>
    /// <list type="bullet">
    ///   <item>
    ///     <b>Tier 0 — Same key</b> (Δnum=0, same letter): score 1.00.
    ///     All 7 pitch-classes shared; tonal-centroid distance = 0.
    ///   </item>
    ///   <item>
    ///     <b>Tier 1 — Relative major/minor</b> (Δnum=0, A↔B): score 0.90.
    ///     Same 7 notes, different tonic; maximum note-sharing.
    ///     E.g. C major (8B) ↔ A minor (8A).
    ///   </item>
    ///   <item>
    ///     <b>Tier 2 — Adjacent number, same letter</b> (|Δnum|=1 on the
    ...
    ///   </item>
    ///   <item>
    ///     <b>Tier 3 — Adjacent number, relative of adjacent</b> (|Δnum|=1,
    ///     A↔B cross): score 0.65.  Compatible but less obvious; many DJs use this.
    ///     E.g. 8A → 9B.
    ///   </item>
    ///   <item>
    ///     <b>Tier 4 — Parallel major/minor</b> (Δnum=±3 on the circular
    ...
    internal static double? ComputeHarmonic(MusicalKey? keyA, MusicalKey? keyB)
    {
        ...
        // ---- Tier 3: adjacent number, relative cross (|Δnum|=1, mode differs) ----
    ...
    /// <summary>
    /// Energy proximity sub-score: 1 − |ΔE| / 9.
