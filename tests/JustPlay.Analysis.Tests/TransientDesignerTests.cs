...
public class TransientDesignerTests
{
    ...
    public void Percussive_AttackPeak_IsBoosted_AtPunch()
    {
        ...
        // diff/slow ≥ 1.0 → gain = 1 + 0.8×1.0×K = 1.48 → peak is substantially higher.
        // A 10 % lift is a very conservative lower bound (the real lift is ≥ 40 %).
        Assert.True(peakPunched > peakNeutral * 1.10,
            $"Expected punched peak ({peakPunched:F4}) > neutral peak ({peakNeutral:F4}) × 1.10");
    }
    ...
    public void SteadySine_IsUnchanged_AtHighPunch()
    {
        ...
        // individual cycles; both average out to the DC rectified level (2A/π).
        //
        // Crucially, the fast follower has a higher τ_release/τ_attack ratio (15/2 = 7.5)
