...
; registers a clean uninstaller. JUST STREAM (JustStream.exe), JUST TAG (JustTag.exe) and the
; headless JustPlayCLI.exe ship in the SAME folder — they share the app's runtime, BASS natives,
; ONNX runtime and keymodel.onnx (one copy each; the CLI is for agent/power-user library
; tagging+sorting).
;
...
Name: "desktopicon_tag";    Description: "JUST TAG desktop shortcut";    GroupDescription: "{cm:AdditionalIcons}"; Flags: unchecked
...
; JustStream.exe + JustTag.exe + JustPlayCLI.exe (+ JustPlayCLI.howto.txt) sharing ONE copy of
; the .NET runtime, the Avalonia natives (libSkiaSharp / av_libglesv2 / libHarfBuzzSharp), the
; BASS natives, the ONNX runtime and keymodel.onnx. build\publish-win-x64.ps1 produces this folder.
...
Name: "{group}\JUST TAG";    Filename: "{app}\{#TagExe}"
...
Name: "{userdesktop}\JUST TAG";    Filename: "{app}\{#TagExe}";    Tasks: desktopicon_tag
